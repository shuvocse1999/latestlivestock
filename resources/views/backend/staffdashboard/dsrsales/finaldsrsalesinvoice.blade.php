...
{{-- 
 ...
 <tbody>
  ...
  $sqty = DB::table("dsrstocks")->where("invoice_no",$data->invoice_no)->where("product_id",$p->product_id)->where('status',Null)->sum("qty");
  $returnqty = DB::table("dsrstocks")->where("invoice_no",$data->invoice_no)->where("product_id",$p->product_id)->where('status',Null)->sum("returnqty");
